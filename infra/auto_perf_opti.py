...
class PerformanceOptimiser:
    ...
    def __init__(self, cpu_min: int = 80, cpu_max: int = 95) -> None:
        ...
        self.cpu_ocr_processes = CONFIG.cpu_ocr_processes
        self.use_gpu = CONFIG.ocr_opts["use_gpu"]
        if self.use_gpu:
            pass
        else:
    ...
    def _optimize_cpu_usage(self) -> None:
        ...
        if cpu_util < self.cpu_min and self.cpu_ocr_processes < physical_cores():
    ...
    def _changes_made(self) -> bool:
        return CONFIG.cpu_ocr_processes != self.cpu_ocr_processes

    def _save_perf_optimisations(self) -> None:
        if not self._changes_made():
            return
        logger.debug("Saving performance optimisations!")
        CONFIG.set_config(cpu_ocr_processes=self.cpu_ocr_processes)
